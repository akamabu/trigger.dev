...
### `onChatStart` consumers

`onChatStart.messages` reflects the **full prior conversation** on a continuation now (was empty-or-tiny before). The runtime loads the accumulator from snapshot+replay before `onChatStart` runs. If you used `messages.length === 0` as a proxy for "this is a new chat," switch to the explicit `continuation: false` flag — `messages` may be non-empty even on an unrelated continuation path.

```ts before
onChatStart: async ({ messages, chatId, clientData }) => {
  if (messages.length === 0) {        // ❌ unreliable on continuations now
